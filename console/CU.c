...
{
    mt_gotoXY(79, 20);
    write(STDOUT_FILENO, "Любицкий Максим Егорович, ИС-242", sizeof("Любицкий Максим Егорович, ИС-242"));
    for (int i = 0; i < 4; i++)
    {
        mt_gotoXY(79, 21 + i);
...
void CU_JUMP(int cell)
{
    sc_icounterSet(cell);
}

void CU_JNEG(int cell)
{
    int value = 0;
    sc_accumulatorGet(&value);
    if ((value >> 14) > 0)
